...
- Paragraph direction and alignment, common text styles, hyperlinks, lists and basic tables are converted into article HTML. Hebrew direction is inferred only when the paragraph has no explicit direction. RTL paragraphs with left alignment are right-aligned; center and justified alignment are retained.
- A Subtitle-style paragraph following the title becomes the article subtitle. The source collection's unstyled summary directly after a title and before publication/byline metadata is also recognized. It appears beneath the article title, not repeated in the body. Internal Heading 2 and lower remain in the body.
...
`NEWSLETTER_DOCKER_IMAGE=<image> node tests/scripts/scheduled-import-management.mjs` starts and removes a disposable container. It verifies multi-batch fetching, failed-file isolation, tracking pagination/edit/removal, re-imports, authorization and continuation with a disabled schedule. Test-only worker and cron fixtures are mounted into that container and are excluded from the production image.
